    ...
    >
      <div className="flex flex-col sm:flex-row items-start gap-6 md:gap-8">
        ...
        <div className="min-w-0 flex-1">
          ...
          {saint.byname && (
            <p className="font-serif text-[15px] italic text-gold/90 mb-1">
              &ldquo;{saint.byname}&rdquo;
            </p>
          )}
          <h3 className="font-sans text-[26px] md:text-[34px] font-bold text-paper leading-[1.05] tracking-[-0.02em]">
            {saint.name}
          </h3>
          <p className="font-sans text-[14px] text-paper/60 mt-1.5">
            {saint.epithet}
          </p>
          <p className="mt-4 max-w-[640px] font-sans text-[15px] text-paper/75 leading-relaxed">
            {saint.shortBio}
          </p>
          {saint.titles?.length ? (
            <ul className="mt-4 flex flex-wrap gap-2">
              {saint.titles.slice(0, 4).map((t) => (
                <li
                  key={t}
                  className="rounded-pill border border-gold/25 px-3 py-1 font-sans text-[12px] text-paper/70"
                >
                  {t}
                </li>
              ))}
          ...
          <p className="mt-5 font-sans text-[14px] font-medium text-gold group-hover:text-gold/80 transition-colors duration-150">
            Read her life →
          </p>
        </div>
      </div>
