...
export default async function CouncilDocumentPage({
 ...
 <h1 className="font-display-serif text-display-sm md:text-display text-paper leading-[1.1] tracking-[-0.01em]">
 {content.title}
 </h1>
 {content.subtitle && (
 <p className="mt-3 font-sans text-body text-paper/65">
 {content.subtitle}
 </p>
 )}

 {/* Source */}
 <p className="mt-6 font-sans text-caption text-paper/45 leading-[1.55] italic">
 {content.source}
 </p>
 ...
 {s.framing && (
 <p className="mt-4 font-sans text-ui text-paper/65 leading-[1.65] border-l border-gold/30 pl-4">
 {s.framing}
 </p>
 )}
 {s.citation && (
 ...
 <div className="mt-5 space-y-4 font-serif text-lede md:text-lede text-paper/90 leading-[1.7]">
 {s.paragraphs.map((p, i) => (
 <p key={i}>{p}</p>
 ))}
 </div>
 {s.notes && s.notes.length > 0 && (
 <ul className="mt-6 space-y-2 font-sans text-detail text-paper/55 leading-[1.6] border-t border-paper/10 pt-4">
 {s.notes.map((n, i) => (
 <li key={i}>· {n}</li>
 ))}
 </ul>
