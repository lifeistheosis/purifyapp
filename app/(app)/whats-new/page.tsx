...
const ENTRIES: Entry[] = [
 ...
 items: [
  ...
  "The Council of Florence, and what an article can now say. The account of Florence is a full one now: what was pressed at the council, what was signed, and what became of it afterward, with a step by step chronology and four images, among them the sketches Pisanello made of the emperor at the council itself and the union decree with its seals and signatures. Getting there meant giving history articles three things they could not express before. A chronology, so a sequence can be read as a sequence. Sources, so a claim can be traced to where it came from. And uncertainty, so that where the record genuinely disagrees the page says so rather than quietly picking a side. Those belong to the article format now rather than to this one article, and a test holds every article to them.",
