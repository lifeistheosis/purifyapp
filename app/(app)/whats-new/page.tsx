...
const ENTRIES: Entry[] = [
 ...
 date: "August 11, 2026",
 blurb:
  "Most of this release is not new work. Light mode has worked since the theme controller landed: it was called Parchment, filed under premium reading modes, and gated behind Pro, which is the opposite of what Purify said in public when a reader asked for it. The search index has been shipping into both app bundles at about 59 KB to open a dialog that on a phone could not open. A file of the Desert Fathers was written for the days when the front door has nothing to read, and was never imported. The function that answers which Hour you are in has sat in the code with no callers, under a comment claiming a page already used it. And every pre-Pascha reading sat one week late, so a reader in Lent was shown the previous Sunday's Epistle and Gospel the whole way from Zacchaeus to Palm Sunday. Two more of the same kind surfaced while these notes were being written, and both are in here: the calendar inside the app had been showing the day the build was cut, for the life of the install, and every link a reader copied in order to share it pointed at their own phone rather than at Purify. This release is mostly Purify handing over what it already had.",
 items: [
    "Light mode is free, and you can find it. On the twenty-fifth of July a reader asked for light mode, and Purify answered in public: it would not sit behind Pro, because being able to read is not a premium feature. The request had come from elderly readers reporting dizziness on the dark palette. The feature was not on the roadmap. It was already built. It had been working app-wide for as long as the theme controller has been in the root layout, under the name Parchment, filed with the premium reading modes, gated behind Pro alongside Candlelight and Monastery, and reachable only from a pill inside the Bible and saint readers. It was built, misnamed and buried. It is called Light now, the free palettes sort first, and one list in the code is the single place that says which palettes every reader gets. The gate is asked per palette rather than once for all four, because a single yes-or-no is how light mode ended up behind Pro to begin with and is how it would get back there. The identifier has not moved, so a reader who had already chosen it is not reset. The badge has come off the group heading and onto the two chips that are genuinely paid: on the heading it told a reader looking for a readable page that reading was a paid feature. Those two are Candlelight and Monastery, and as of this release they are Purify Plus rather than Purify Pro. Premium Reading Modes was both listed and gated as a Pro perk; it is a Plus one now. Nobody loses anything by that, because a Pro account already satisfies the Plus gate, and the pricing page no longer sells Parchment, which is the free Light palette under the name it used to carry.",
    "The Lenten lectionary was a week late, all of it. Every pre-Pascha offset in the movable readings sat one Sunday late. The five Sundays of Lent belonged seven to forty-two days before Pascha and were keyed seven days short of that, and Palm Sunday was keyed five days before Pascha rather than seven. On screen that meant a reader in Lent was shown the previous Sunday's Epistle and Gospel, every Sunday, the whole way from Zacchaeus to Palm Sunday, and Lazarus Saturday landed on the Friday. The rest of the calendar had the arithmetic right the entire time: the fasting status and the season both compute Palm Sunday and Clean Monday correctly from Pascha. The data file was the lone outlier, which is why nothing else disagreed with it and nothing caught it. Fixing the keys also filled two holes they had been hiding, so Great and Holy Monday through Great and Holy Saturday now have their own readings instead of falling through to nothing.",
    "The calendar showed the day the build was cut. On Android and on iPhone the calendar opened on the day the build was cut, in the month the build was cut, for the life of the install. Nothing a reader tapped moved it: not a tile, not the month arrows, not the app's own idea of what today is. Inside the app there is no server, so a screen that asks what day it is while it is being built receives one answer and keeps it, and the address parameters that would have corrected it are forced empty there. The Bible tab and the daily prayers were moved onto the reader's own clock in Beta 2.7. The calendar was the screen left behind, and the test that exists to catch exactly this carried a written exemption for it, recording the debt rather than paying it. The exemption is gone, so that guard now covers this page and will fail if it ever slips back. One thing is deliberately given up to get there: the teaser that printed the text of the appointed passage. It read a whole chapter from the bundled files, and a page that can be asked for any day of any year cannot do that without a server, so the readings band shows the citation and a link, which is what the day panel and the Bible tab's appointed block already do. The same screen was carrying 51 complete saint biographies, with their lives, works and quotations, in order to paint 42 tiles that show a name: 151,385 of the page's 201,837 characters. A tile now carries only what a tile paints, the date of Pascha is worked out once for the year rather than once for every tile, and two chapters nobody displayed are no longer read at all. The page is 43,435 characters. The grid has also come off pure black, which had drawn a hard seam across the middle of the screen against the page above and below it, and its loading skeleton now matches the page it stands in for, rather than promising one shape and then moving it.",
    "A copied link pointed at the phone that copied it. Every copy-link and share control in the app built its address from the page the reader was standing on. Inside the app that address is not purifyapp.net: Android serves the bundled app from https://localhost and iPhone from capacitor://localhost. So a verse a reader copied and sent to somebody opened nothing on the other person's phone, and nothing on their own once the app was closed. Four places did it: the Verse of the Day card, the verse menu in the Bible, the paragraph menu in the saints, and the share control that hands a link to the phone's own share sheet. This is the same fault that once put a localhost address into a confirmation email, one surface over, and it is answered the same way. One function now decides the address a shared link carries, and outside development it refuses the local one however that origin is spelled, because a copied link exists only to leave the device. Anything handed to it keeps its page and loses its host, so a link cannot carry localhost back through by another route. Fifteen tests hold the behaviour, one of which reads the source of every component and page and fails if a new share link is ever built the old way. The architecture note for highlighting had stated that the single outward link from a highlight pointed back at purifyapp.net. It did not, on any device, and the correction is written into that note rather than quietly swapped.",
    "The header stopped moving when you leave the home page. Reported as two complaints, that the buttons are not aligned and that the spacing changes when you leave the page. They were two different faults. Discover sat lower than every word beside it, by 2.67 pixels, because it was the one item in the row wrapped in a plain block instead of being a link in its own right, so it carried the taller line height of body text and the row was then centring two different heights. Separately, the home page uses one header and every other page uses another, and the two disagreed on four measurements. The one a reader could see was a left cluster of different width, which started the wordmark and every link after it 14 pixels further right on every page but home. All four values match now. Measured from the content edge, both headers put the wordmark at 51 pixels and the link row at 237.16.",
    "Search, on a phone. Purify ships a search index of about 59 KB into the Android and iPhone bundles. The dialog that reads it was unreachable on a phone twice over: the keyboard shortcut that opens it returned early unless the window was at least 768 pixels wide, and the dialog itself was hidden below that width. So every phone reader has been carrying a search index for a search that could not open, on the platform where three separate per-screen searches were the only way to find anything by name. Both gates are gone and there is a search control in the header of Today, Bible, Discover, Prayers, Reading and You, and in the four reader screens. The dialog also moved to the root layout, because Today sits outside the app group and was therefore the one screen it did not exist on at all. That is the fifth thing to move for that exact reason.",
    "The Fathers, made readable. A reader said the commentary was too long and written in difficult old language. Measuring it turned one complaint into three problems, and only the last is a matter of taste. There are 4,479 notes. The median is 294 words, the ninetieth percentile is 2,059, and the longest single note was 97,501 words. First, two of those notes were not text at all: they ended in the scraper's trailing index of links, thousands of lines of file paths, shipped to readers as St Augustine's commentary. The note on Psalm 150 was 76 percent link dump, 19,141 words of it. It is 5,871 words now, and no file path of that kind remains in any of the 4,479 notes. Second, opening a card handed over the entire note. A preview now stops at the first paragraph break past 220 words, so a median note arrives essentially whole and only the tail is held back, with the number of paragraphs remaining and the reading time shown before you tap, because someone deciding whether to open an eight hour homily on a phone deserves to know that it is one. Third, the hard words are glossed.",
    "Five days in six, the front door had nothing to read. Probing the daily resolution across the whole of 2026: sixty days land on a saint with a written profile, and 305 fall back to a name, a placeholder glyph and a link to the calendar grid. Theophany, the Synaxis of the Forerunner, the Circumcision and St Sava are all among the misses. Five days out of six the front door of the app had nothing on it to read. A file of twelve sayings of the Desert Fathers, public domain, from Budge and Waddell, attributed line by line, was written for exactly this and never imported. Its own header describes the job it was meant to do. It appears now, on those days only, and it appears as its own card with its own heading and the Father named on the quotation. That is the whole design decision rather than an incidental one: a quotation sitting underneath a commemorated saint's name reads as that saint's words, and misattributed patristic text is the one absolute prohibition in Purify's editorial standards. On the days when the saint of the day has a profile, nothing changes.",
    "Which Hour you are in. A reader opens the Hours with one question: which one is mine right now. The page answered it nowhere, offering five near-identical rows separated only by a clock time the reader had to compare against their own watch. The answer was already written and had never been called from anywhere, under a comment claiming the prayers landing page used it to highlight which Hour the day was closest to. No page ever did. That comment has been corrected rather than left to mislead the next person to read the file. The mark is a word rather than a colour, because colour is never the only signal here, and it states which Hour the day is in and then stops: no count, no streak, no nudge to pray. The clock is read after the page loads rather than while it is being built, because this page travels inside the app, and a time baked in at build would have marked the same Hour for the life of the install.",
    "The entrance fade was trapping every dropdown on the page. Reported from the Android build on the ninth of August: searching the saints for Paisios returned the right names and drew them underneath an icon of the Theotokos. The list was there, and unreadable. The gentle fade that brings a screen's sections in was holding itself in effect after it had finished, which quietly made every section a law unto itself for the purposes of what may paint over what. So the search results, which ask to be drawn above everything, could not reach past the block of saint icons that follows them. The fade now releases the element the moment it is done. This is not only the saints page: the same fade is used on screens throughout the app, so every dropdown, popover and menu inside one was subject to it.",
    "The button that said Open Purify did not open Purify. The main control on the front page was labelled Open Purify, and what it did depended on whether the app was already installed. For a first-time visitor it was not, so the button disabled itself for a second and a half and then either raised the browser's install dialog or opened an explainer. The plain link into the library existed only in the branch a new visitor by definition never reaches. The most motivated person on the page, the one who came to read and pressed the button offering exactly that, was handed an install prompt for software they had not yet read a word of, and the actual way in was the quieter secondary link underneath. The button opens the app now.",
    "The padding meant to keep the last verse clear was removing it. Three helpers exist to hold content clear of the tab bar and the home indicator inside the app. On the website they were meant to do nothing, and were written to do nothing by setting the spacing to zero. Setting something to zero is not the same as leaving it alone. Those rules were emitted after the ordinary spacing they sat beside and won against it, so on a phone browser thirteen surfaces silently lost the spacing they had asked for: the last verse of every chapter sat flush against the bottom edge of the screen, and three fixed bars in the shop had their buttons on the edge. On a wide screen a second rule landed later still and covered it up, which is why this was invisible on a computer and went a whole release unseen. The helpers are gone rather than corrected, because the versions that run inside the app are ordinary rules that were already winning on their own. Nothing about the app changes. On the website the spacing each page asked for now simply applies. The same mistake had been made with the height of the tab bar, which was written as a fixed 86 pixels for every reader although the bar exists only inside the app, so four floating controls on the website were positioned 86 pixels above a bar that was not there.",
    "A Plus renewal could quietly revoke a Pro membership. Entitlements are written by overwriting the whole row. The subscription webhook knew that for one field and preserved it, but it computed the Pro expiry as either the Plus date or nothing at all. So any event that did not itself carry Pro wrote nothing over Pro. In practice: grant someone Pro for the EIKON Box, and their next ordinary renewal erased it. No cancellation, no refund, no notice, and nothing left in the row afterwards to show it had ever been granted. Pro is the tier with the monthly mailed icon and free shipping attached, so this is the one with physical fulfilment behind it. An event now speaks only for the products it actually carries, and when Pro is not among them the existing Pro date is carried forward untouched. Pro still ends on its own expiry.",
    "Two numbers were reporting better than reality. Neither is a crash, which is why both survived: they simply read well. The shop's star rating counted reviews seeded by the operator. The code already knew those rows exist and had already stopped them wearing the verified badge, citing the fabricated-review practice this project declined once before. The badge was fixed and the average was not, so a seeded five still lifted both the star average and the review count, which are the two numbers a shopper actually reads before buying. Both are computed from verified rows only now, using the same test that governs the badge, so the number and the badge cannot disagree. Every review is still shown. Separately, gifted subscriptions were counting toward monthly recurring revenue, which is a number nobody pays.",
    "iPhone has its own Plus switch now. One flag decided whether Purify Plus was enforced, and both store builds read it, because it was derived from a single marker that Android and iPhone both send. The same repository secret was passed to both build workflows. Throwing the Play Store launch switch, which is the last step on the Android launch checklist, would therefore have enforced Plus in the next iPhone build as well. On iPhone the two subscription products are not attached to any submission and cannot currently be bought, so readers there would have lost sync, the Florilegium, the ambience and Immersive History with no way to purchase any of it back. That is precisely the harm the website's own separate switch was created to prevent. The three surfaces are now told apart properly and each has its own switch. Nothing is enforced anywhere in this build; this is a guard against a future step, not a change a reader will see.",
    "The saint icons say where they came from, or say that they do not. One hundred and ten icons ship inside the app, and 107 of them had no source, no licence and no attribution recorded anywhere. Every other class of image in Purify, the history media, the section media and the shop media, is required to carry the work, the artist, the date, the source, the licence and a link, and a test enforces it. This directory required nothing. There is a record for all 110 now, and it is deliberately in two parts. Three rows are settled. The other 107 state only what the bytes and the registries actually prove: which registry points at the file, what the file truly is, its dimensions and its size. No licence, no artist, and no guess dressed up as a fact. Those 107 are still unverified and this note is not claiming otherwise. Groundwork for it turned up a file that was a WebP wearing a .png extension, at 450 KB the largest in the directory, which the image optimiser had been skipping because it decides what to do from the extension. Re-encoded honestly it is 72 KB. Five filenames contained characters that do not survive being copied onto an Android asset path.",
    "The release notes fold into years and months. This page carried 89 releases as one flat column of 43 days, newest first, and never said what year any of them were in. Finding something from May meant scrolling past four months of headings to reach it. It nests now: a year, then a month inside it, then a day, then the release and its full list. The most recent day is still open when the page loads and everything else is still closed, so the first thing on screen has not moved. Expand all and Collapse all reach every level. A release still carries one date, written once, and the year and month are read back out of that rather than recorded a second time, because two copies of the same fact are two chances to disagree.",
    "Housekeeping, and what is not yet done. The support address moves to lifeistheosis@gmail.com in nine places, among them the Terms page, the shop policies, both seller screens and the reply address on order email. The stored copy of it in the shop database moves with a new migration that has to be applied by hand, so until that is applied the old address is still what the shop's own record holds, and it should not be assumed otherwise. Four placeholder dashes that a reader or a seller could actually see are replaced with words that say what the blank means, Not recorded, Not available and None, which reads better than a dash in any case. Fifteen operator-only screens still use one and are deliberately left until a pending decision about the operator design is settled, rather than sweeping cosmetics through files that may be rewritten. There is a new design preview behind its own admin route, which replaces nothing and is visible to nobody but the operator. The test suites that read the content corpus were timing out for no reason other than their own size on a loaded machine, roughly two runs in three on Windows, so a suite that was red for no reason was training everyone to ignore red; they are given the time the corpus actually needs. The cache version steps to purify-1.1.0, so a returning reader is served this build rather than the last one. Prayer Campaigns and Conversations are built out considerably further in this build, and are switched off in it. Campaigns gain a record of which local days a reader actually kept, rather than a count of hours that refused a second prayer at eight in the morning and allowed one at four; a streak read from that record, forgiving today so the number never reads zero at breakfast; private groups within a campaign, reached only by a six-character invite code, with no directory and nothing to rank; and one optional reminder a day, carrying no campaign title and no name, because a campaign is often for somebody's surgery and that is not a sentence for a lock screen in a shared room. Conversations gain a panel that can be linked to, an inbox that reaches the post it names, replies that appear at once and say so when they fail, a badge that lights and clears while the app is open, and a feed that refreshes. None of that is something a reader meets in this build. Both surfaces stay behind their own switch, both switches are off, and the two new migrations dated the eleventh of August have to be applied by hand before either may be turned on. They are written down here because they are in the build, not because they are available.",
 ],
 },
 {
 version: "1.0",
