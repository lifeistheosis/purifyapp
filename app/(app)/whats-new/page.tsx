...
const ENTRIES: Entry[] = [
 {
 version: "Beta 3.0",
 kind: "Several things were not happening",
 date: "August 2, 2026",
 blurb:
  "Some of this is new: the whole of August's saints, the Council of Florence at full length, and the Today page rebuilt for the computer screen it actually opens on. More of it is things that were supposed to be happening and quietly were not. The app had never reported a single session. Prayer reminders had no scheduler to send them. Accounts created through Google were never shown the Terms. And a Father's page never told you when the words on it were not his.",
 items: [
  "Saints' pages now say whose words you are reading. A Father's page is set in serif, and a reader skimming it is entitled to assume the serif is the Father speaking. On a number of works that assumption was wrong, and nothing on the page said so except a sentence at the end of a long editor's note. The Martyrdom of Polycarp is the church of Smyrna writing about him. St Nicholas's troparion is the Church's hymn to him. The miracles in the Life of Antony are Athanasius narrating. Each of those sections now carries a quiet band above it naming whose words follow, set in sans against the reading serif so a skimmer registers it without being shouted at. Where the words are the saint's own, which is the usual case, it says nothing at all. The same declaration is what search engines and machine readers are given now, so a work carrying our own retelling is no longer published as a book he wrote. That is the one class of mistake nothing ever corrects, because it is copied and repeated.",
  "Today, on a computer. On the web there is no Today tab. The daily prayer page is the only Today the site has, and it had been showing less of the day than the phone does, laid out for a narrow column. It now opens with the date and the day's fast set against it, then the one rule the hour asks for with a button to begin, then the Verse of the Day, which the web did not carry at all. Beside them, in their own column, is the Church's own reckoning of the day: the saint with their icon, the fast in full words, the appointed readings, and the count to Pascha. Those are drawn by the same part of the app the phone uses, so the two can no longer disagree about what day it is. The page is a third shorter than it was and holds more, and all of it is visible without scrolling on an ordinary laptop.",
  "The Council of Florence, and what an article can now say. The account of Florence is a full one now: what was pressed at the council, what was signed, and what became of it afterward, with a step by step chronology and four images, among them the sketches Pisanello made of the emperor at the council itself and the union decree with its seals and signatures. It is also the first article with a chronology, which is new to the format rather than to this one piece: a dated list of the steps of something that ran for months or years, set on its own rule so the shape of it can be seen without reading the narrative twice. It is a numbered list underneath, so it reads in order to a screen reader and can be found by a search engine. Every history article already carried its sources, and nineteen of the forty already said where the record is uncertain. Florence does both, at the length the subject asks for.",
  "August has its saints. The whole month, in two passes: eleven for the first part and the rest after it, each with a Life, and, where a public domain text exists, their own writings to read in full. Cyprian of Carthage arrives with four, among them On the Lord's Prayer and On the Mortality. The Abgar documents come with Thaddeus of Edessa. Three faults in the tooling that prepares those texts were caught by the work itself and fixed.",
  ...
  "Accounts made with Google were never shown the Terms. Creating an account through Google presented no Terms notice of any kind, and recorded no acceptance, while the two email forms did both. Signing in and signing up with Google now carry the notice, and acceptance is recorded once per version of the Terms rather than repeated on every later sign-in.",
  "The Android app had never reported a session. Not a screen view, not a session, nothing, for as long as it has been installed on anything. The cause was narrow. The app runs from its own address on the device and posts to purifyapp.net, and the endpoint receiving those posts refused anything that did not come from the website itself, so every pageview from an installed app was turned away. The endpoint now recognises the app's own shells by name, and nothing else.",
  "Prayer reminders had nothing to send them. Two scheduled jobs existed in the code, one to deliver prayer reminders and one to record the day's donations, and nothing anywhere called either of them. Reminders never fired and the record was never written. There is a scheduler now, kept in the repository so it can be read and changed like everything else. Sending still waits on its keys being set, so what arrives here is the mechanism rather than the notifications.",
  "A run of Android repairs. The app was registering the website's background worker, which could serve it a stale copy of the site instead of its own. Reinstalling now gives a clean slate rather than restoring what was there before. The gap above the keyboard is no longer a grey box. Spacing around the notch and the home bar is handled by one thing instead of two disagreeing.",
  ...
  "The Today page can be read by more people. Every small label on it sat at a contrast of under four to one against the background, which is below the readable standard, and they are above seven to one now. The page had one heading and then eight unheaded blocks, which a screen reader met as a wall, and each section carries a real heading now. Everything on it that can be tabbed to shows where the focus is, which none of it did.",
  "Housekeeping. A developer panel for the accounts on the allow list, which can turn on test premium and individual feature flags without touching a real subscription. The cache version steps to purify-beta-3.0.0, so a returning reader is served this build rather than the last one.",
 ],
 },
 {
 version: "Beta 2.9",
