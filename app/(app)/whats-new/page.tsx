...
const ENTRIES: Entry[] = [
 {
 version: "1.3",
 kind: "A way back, and a saint who was only a name",
 date: "August 28, 2026",
 blurb:
  "A reader wrote in the app's own Community tab that on iPhone, once you tap into a section, getting back sometimes means leaving the section altogether and finding your way down again. Sometimes undersold it. Eighty nine of the app's ninety nine screens had no way back at all, and on the website the browser's own back button had been covering for it, so it never showed up as a bug. Every one of those screens has one now. The rest of this release is the same kind of thing: work that was nearly finished and did not quite reach you. Seven prayer categories that said Planned and nothing else. A tab bar carrying seven tabs. A Spanish tab with no word on it. St Arsenius the Great, kept every May 8 and named on the day, with nothing behind his name. And five saints the calendar had been quietly listing twice. This release also carries the first of the Communion prayers, the day's commemoration on the Android home screen, and a sign up that can no longer hang.",
 items: [
    "A way back, on every screen. The app had a back button and it was fitted per screen, which meant it was fitted on ten of them: five pages that carried their own header, and the five account pages that inherited one. The other eighty nine had nothing. On the website that never mattered, because the browser has its own back button and quietly stood in. Inside the app there is no browser and iPhone has no hardware back, so a reader who tapped from a saint into one of his writings was stuck there and had to leave through the tab bar and walk down again. That is what was reported and that is what is fixed. The button knows where it is: it stays off the tab screens, where back means nothing, and off the ten screens that already had one, so nothing shows two. If you opened the app straight onto a page from a link, where there is genuinely nothing behind you, it takes you to Today rather than doing nothing at all.",
    "Six tabs, and the account moves to Discover. The bottom bar carried seven tabs, which left each one about fifty eight pixels wide and made it the most crowded thing in the app. You has left the bar. Your account now sits in the Discover library as Settings, keeping the same gear it always wore so it reads as a move rather than a disappearance, and Discover stays lit while you are in there. Discover also lost its Featured today panel, which picked a topic and a council to show you. In the app that pick was frozen when the release was built, so the today in its name was never true there. Orthodox History was already in the Discover library and had been lighting no tab at all when you opened it. It does now.",
    "Like on Community. Posts and replies can be liked. Dislike is there too and it is recorded, so moderation has the signal, but only the like total is shown. A visible dislike tally on a prayer request invites exactly the pile on it measures, and a number people have already seen cannot be taken back. Pressing what you already pressed takes it back, and pressing the other one moves you across without making you undo the first. The count moves the moment you press and the server corrects it, so a slow connection never leaves a like on screen that was never recorded.",
    "St Arsenius the Great. He was kept on May 8 and named on the day, and there was no page behind the name. The Roman deacon whom Theodosius the Great chose to teach his sons, who left the palace for the desert of Scetis and became the Church's great teacher of silence: flee, be silent, pray always. He has a life now, and the sayings he is remembered by, including the one the fathers repeated most, that he had often repented of having spoken and never of having been silent. He is the one hundred and fifty eighth saint in the library. He carries no icon yet, so his page shows the same placeholder as the others still waiting for one.",
    "Five saints were listed twice. Adding Arsenius turned up a fault that had nothing to do with him. The calendar names some saints in plain text and links others to their page, and when a saint had a page but the day's entry had not been linked to it, the day showed him twice: once as text you could not tap, once as a link directly underneath. It was happening to St Silouan the Athonite on September 24, St Job of Pochaev on October 28, St Porphyrios of Kavsokalyvia on December 2 and St Herman of Alaska on December 13. St Paisius Velichkovsky was a different case: he had been entered twice on November 15 with two different notes. All five days now name each saint once, and the app will refuse to build if it ever happens again.",
    "Planned prayers say what is coming. Seven prayer categories are listed but not yet filled, because the text for them has to be a real public domain translation and nothing is ever invented to fill a gap. They were marked Planned and said nothing else, so tapping one did nothing and gave no reason. Each now says what it will hold: before work or study, after work, before travel, the Nativity fast, the fasts of the year, psalms for sickness and fear and sorrow, and thanksgiving. They still do not open, because there is still nothing to read. They will not describe a service that has not been sourced, and the app now refuses to build if one tries to.",
    "The prayers before Communion. A reader asked in Community for the prayers of preparation before receiving, and for the thanksgiving afterwards. The first half is here. Hapgood's Service Book prints six prayers of preparation and all six are in the app: St Basil the Great, St John Chrysostom, Simeon Metaphrastes, Simeon the New Theologian, St John of Damascus, and the short prayer said immediately before receiving. They are her words in her order, from the 1906 edition, which is out of copyright. Nothing was smoothed or modernised, and nothing was written to fill a gap. Where the scan had broken a word across a line, it was rejoined only where that same word stands unbroken elsewhere in the volume, and the ingest stops rather than guess at one it cannot find. The thanksgiving after Communion is not here yet. It will arrive the same way or not at all.",
    "Eight places where the scan showed through. Purify's texts are taken from scans of printed public-domain volumes, and where the scanning software cannot read a mark it prints something else in its place. Eight of those had reached the page. In the prayer of Simeon Metaphrastes, said in preparation for Communion, three questions stand one after another, and all three had lost their question marks: two black squares, a stray bracket and a hyphen were standing where they should have been, so a man asking what evil he has not done appeared instead to trail off. The same prayer ran two words together in the name of the Holy Spirit. St John of Damascus carried two stray carets in the middle of his sentences, left behind when the number of a footnote was taken out and the mark that pointed to it was not. A section of St Cyril on Luke was headed with the code for an ampersand rather than an ampersand. And the last note on the fourteenth chapter of Matthew ran on into the heading of the fifteenth, which is the same fault the last release took out of the Fathers' mouths in five other places. All eight are repaired against the volumes they came from rather than patched over, and every one was confirmed on the page before it was touched. The app now reads its whole library looking for that kind of damage, four thousand six hundred files of it, and refuses to build if it finds any.",
    "The day's commemoration on your home screen. Two readers asked for a widget. On Android there is one now: the saint the Church keeps today and whether the day is a fast, on the home screen, without opening the app. It shows the New Calendar reckoning. The table behind it carries both, and the day it names is computed by the same code the app uses, but there is no way yet for your calendar choice to reach the widget, so an Old Calendar reader sees the New Calendar day until that is wired. It turns over at midnight where you are, not where the build machine was, and it survives a change of time zone. It needs no connection at all, because the whole year is worked out when the release is built and carried inside the app, four hundred days of it, so a reader who never updates still gets a correct day for longer than a release cycle. There is no iPhone widget yet. That one needs a separate piece of the app which does not exist today, and it is better to say so than to let it look forgotten.",
    "Tab labels in your own language. Three faults in the bar at the bottom, which is the most looked at text in the app. In Spanish the You tab had no word on it at all: the label was present but empty, which beats the fallback to English that a missing label would have got. Shop was written in English for everyone, in all twenty one languages, even though the word had been translated for every one of them. And Community had only ever been translated into English, so twenty languages showed the English word. All three are fixed, and the app now refuses to build if any label in any language is blank or falls back to English.",
    "Sign up stops being able to hang. A reader reported the page freezing while creating an account. Before an account is made, the app records that you accepted the terms, and it waits for that to be written. Nothing bounded the wait. On a poor connection the button could read Creating account until the browser itself gave up, which is a wait with no end and no way out of it. Inside the app it is worse, because that call crosses to the website and fails more often than the same call made on the site. The wait is now capped at twelve seconds, past a slow mobile round trip and well short of the point where somebody force quits. If it does not get through you are told to try again, which was always the only other outcome: sign up is deliberately stopped when acceptance cannot be recorded, so hanging served nobody.",
    "Twenty one languages, finished. The app has offered twenty one languages for a while and only English was complete. The next best was German at nineteen per cent, and eight languages sat under nine. Nothing looked broken, because anything missing falls back to English without saying so: a reader in Belgrade or Athens simply found most of the app in a language they had not chosen, after a picker that promised otherwise. Thirty five thousand five hundred and eighty four missing strings are now filled, fifty three thousand across the twenty languages in total, and every one of them was checked before it landed. Finishing the catalogs was not enough on its own, because some of the app never went through them. The prayer rope showed its four prayers in English, so a reader counting knots prayed in English while the same prayer stood translated one screen away. On a phone the Bible index listed Genesis and Matthew in English while the same list on a computer named them correctly, because only the phone had been missed. The sentence above the sign in buttons, the one that makes continuing count as agreeing to the terms, was English for everyone, which is worth more than tidiness: notice a reader cannot read is worth less than none. And the prayer index, the names of the morning rule and Small Compline and the Canon, came out of a table that held English and German and nothing else. Five hundred and fifty nine strings across sixty screens now read from the same catalog as the rest of the app. Counting is not done the same way everywhere and the app now knows it. Arabic has six forms for a number and uses all six, Russian and Polish and Ukrainian need three, and three hundred and forty three of those forms now exist in the catalogs and never will in English. The date above the prayers follows the reader's language too, in that language's own order, so the thirtieth of August is said the way that language says it. All of it is now checked on every build. A missing word, a blank one, a number lost out of the middle of a sentence, an em dash, or a stray piece of HTML markup will stop the build rather than reach a reader.",
    "Four things a reader found. A new icon in the shop could not be opened in the app. Tapping it put you on Today, with no error and no explanation, and the odd part was that you could still add it to the cart from the card, which made it look like the product existed and its page did not. That is exactly what was happening. The app carries a page for every icon that existed when the release was built and nothing for one added afterwards, so the shell fell back to its own front door. Icons now open through a fixed address that is always in the app and carries the icon name alongside it, so anything added to the shop opens from now on, including one renamed after a release. The website never had this, because it builds a page on demand and always could. The cart panel had its heading under the phone status bar. It slides in over the whole screen and was measuring from the very top of the glass rather than from below the clock and the battery, so on any phone with a notch the words sat in the strip that is covered. The sign in page had the same fault in a different place: it is the one screen in the app that was never given that measurement at all, so the Purify wordmark and the Privacy link were tucked under the status bar there too. Both now start below it, and on a phone without a notch nothing moves at all. And the saint bump did not work while signed in. Asking for a saint's writings is the one place a reader can tell us what to translate next, and in the app it showed Sign in to request to readers who already were. The app is built before anyone signs in, so the page was deciding at build time that nobody was, and the answer was baked in. It now checks when the page opens instead. Three other things in the same path were fixed with it, because any one of them would have left the button working and the request still failing: the app was calling the site without saying who it was, the server was looking for a signed in reader in the one place the app does not put it, and the browser permission check had nothing to answer it. Worth naming one a reader would have felt: the app also believed nobody had ever bumped anything, so a reader who had already asked for a saint would have seen the button unpressed, pressed it, and quietly withdrawn their own request.",
 ],
 },
 {
 version: "1.2",
