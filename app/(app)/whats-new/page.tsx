...
const ENTRIES: Entry[] = [
 ...
 items: [
    ...
    "Sign in with Apple is built, and is not switched on yet. The sign-in screen has carried a greyed-out Apple button labelled coming soon since it was built, and the connected-accounts screen said coming soon even to readers whose account was already linked through Apple. Both of those are gone. What replaces them is written and tested but waits on one piece of configuration that lives outside the app, and until that is in place the button could not finish what it started. So rather than swap a button that says coming soon for one that fails, Purify shows neither, and the real one appears the moment it can work. It will be offered on the website and on the iPhone app, and not in the Android app, where Apple gives no proper way to do it. One thing did change today regardless: the connected-accounts screen now tells the truth, so an account already linked through Apple is described as connected rather than as coming soon.",
