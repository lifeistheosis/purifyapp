...
describe("isVerifiedBuyer (verified-buyer review rule)", () => {
  it("allows a review when a PAID order contains the product", () => {
    const orders: PurchasedOrder[] = [
      { payment_status: "paid", product_ids: [OTHER, P] },
    ];
  ...
  it("rejects when the only order containing the product is not paid", () => {
    const orders: PurchasedOrder[] = [
      { payment_status: "pending", product_ids: [P] },
      { payment_status: "refunded", product_ids: [P] },
    ];
    expect(isVerifiedBuyer(orders, P)).toBe(false);
  });

  it("rejects when paid orders exist but none contain the product", () => {
    const orders: PurchasedOrder[] = [
      { payment_status: "paid", product_ids: [OTHER] },
    ];
    expect(isVerifiedBuyer(orders, P)).toBe(false);
  });
  ...
  it("ignores null product ids from deleted products", () => {
    const orders: PurchasedOrder[] = [
      { payment_status: "paid", product_ids: [null] },
    ];
    expect(isVerifiedBuyer(orders, P)).toBe(false);
  });
});
