...
description: >
  Build newsletter writing instructions inside a Cowork project. Runs after voice-builder. Produces docs/brand/newsletter-voice.md, a single file Claude references when drafting newsletters in the user's voice. Works with or without existing newsletter samples: if the user has past issues, the skill analyses them; if not, the skill offers 6 archetypes tuned to the user's voice. Trigger whenever the user says "build my newsletter voice", "learn my newsletter style", "set up my newsletter system", "train on my newsletters", "newsletter onboarding", or drops newsletter samples into chat asking for an analysis. Requires voice-builder to have run first: the skill needs docs/brand/voice.md and docs/brand/about-purify.md in the project to work.
---

> **Purify overlay.** This skill is installed in the Purify repo and writes under Purify's name, "Edgar, the Purify Team".
> Read `docs/brand/voice.md` and `docs/brand/about-purify.md` before drafting. **Section 0 of voice.md is binding and overrides
> every instruction below it**, including any instruction here to sharpen a hook, raise a score, or make a claim land harder.
> Hard limits: never invent, restore, or paraphrase-as-quotation a patristic or Scriptural text; never settle a disputed
> doctrinal question; never present AI-drafted framing as Church teaching; never claim a feature that is dark in production;
> never state a price without checking `lib/premium/plans.ts` against what Google Play actually charges. No em dashes, ever.
> Full rules: `docs/editorial-standards.md`.
...
The moment this skill is triggered, check the project root for docs/brand/voice.md and docs/brand/about-purify.md.

If either file is missing, tell the user:

> Newsletter voice sits on top of your general voice profile. Run voice-builder first (upload the skill or say "build my voice"), then come back here once docs/brand/about-purify.md and docs/brand/voice.md are in the project.
...
After the user picks an archetype, load the matching defaults from `references/archetypes.md` inside this skill folder. Tune every field using docs/brand/voice.md and docs/brand/about-purify.md before writing docs/brand/newsletter-voice.md. Flag inside the output file that archetype defaults were used and the file should be revisited after 5 published issues.

## Step 3. Write docs/brand/newsletter-voice.md

Create docs/brand/newsletter-voice.md in the project root. Single file, 800 to 1,200 words target. Use this structure:

```
# Newsletter Voice

## Source
[Sample-based: analysed X newsletter issues] OR [Archetype-based: [archetype name] tuned to docs/brand/voice.md. Revisit after 5 published issues.]

## Audience and purpose
[Who reads this newsletter and what they get from it. Written from docs/brand/about-purify.md and docs/brand/voice.md. 2 to 3 sentences.]

## Voice principles
[3 to 5 core principles the writing always holds. Each one a short declarative sentence. Tuned to this user's docs/brand/voice.md.]
...
> Your newsletter voice is built. docs/brand/newsletter-voice.md is in your project root alongside docs/brand/about-purify.md and docs/brand/voice.md. When you want to draft an issue, say "write a newsletter" and I will use all three files together.
>
...
- docs/brand/newsletter-voice.md: newsletter-specific writing instructions covering audience, voice principles, opening formula, section flow, data philosophy, formatting, closing, absence patterns, and length targets

## Rules

- Require docs/brand/voice.md and docs/brand/about-purify.md in the project root before running. Stop and redirect to voice-builder if either is missing.
- Minimum 2 newsletter samples if the user chooses sample-based mode. Offer archetype mode if fewer.
- Keep docs/brand/newsletter-voice.md under 1,200 words. Tight beats exhaustive.
- Do not invent voice signals. Work only from samples or archetype defaults tuned to docs/brand/voice.md.
- Do not duplicate content from docs/brand/voice.md. Reference it where relevant. docs/brand/newsletter-voice.md adds newsletter-specific rules only.
...
- Do not produce a separate voice or banned-words file. Absence patterns live inside docs/brand/newsletter-voice.md as a single section.
