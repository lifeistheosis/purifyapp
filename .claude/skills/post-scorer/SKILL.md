...
Read docs/brand/about-purify.md and docs/brand/voice.md from the project if they exist. If missing, note it and score without voice matching.
...
[
  {
    ...
    "options": [
      ...
      {"label": "Use reference benchmarks", "description": "Score against the built-in LinkedIn benchmarks in this skill (published creator baseline, 500 posts analysed). No cost, no scrape."},
...
Use the benchmarks listed at the bottom of this file. State plainly in the output that the baseline is a general creator benchmark, not Purify data.
...
If docs/brand/voice.md exists:
- Does the post match tone, rhythm, sentence length from docs/brand/voice.md?
- Does it violate any rule in docs/brand/voice.md's absence patterns section (what the voice never does)?
...
- Are there any red flags: banned words listed in docs/brand/voice.md's absence patterns, generic phrases, corporate tone?
...
Reference benchmarks, from the upstream skill author's published account (350k followers, 500 posts). They are a general creator baseline, not Orthodox-audience data, so treat them as directional:
