...
The manual identities are gone from the project. Removing them was necessary and
not sufficient, because the next run showed what automatic signing actually
does:

```
Communication with Apple failed: Your team has no devices from which to
generate a provisioning profile.
No profiles for 'net.purifyapp.purify' were found: Xcode couldn't find any
iOS App Development provisioning profiles matching 'net.purifyapp.purify'.
```

**Automatic signing archives against a *development* profile**, not a
distribution one. It signs the archive for development and expects
`-exportArchive` to re-sign for distribution afterwards. Development profiles
require at least one registered device, and this team has none, wants none, and
has no reason to have any: nothing here is ever installed by cable. So the
automatic path cannot work for this project at all, and the error says so only
if you already know that development profiles need devices.

The archive therefore signs **manually** against an App Store profile, which
carries no device list. The profile is not a pasted secret. The workflow mints
it on every run from the App Store Connect API key it already holds, using
`scripts/ios-provisioning-profile.py`, which:

- finds the App ID and every Apple Distribution certificate on the team,
- deletes any previous profile named `Purify App Store (CI)` and issues a fresh
  one, because a profile is a frozen snapshot of the App ID's capabilities at
  the moment it was issued, and reusing one is how a build quietly loses an
  entitlement it is supposed to carry,
- prints whether the new profile actually grants `aps-environment` and
  `com.apple.developer.applesignin`, so a capability turned off in the portal
  shows up here rather than as push that never arrives.

`CODE_SIGN_STYLE=Manual`, `CODE_SIGN_IDENTITY`, and
`PROVISIONING_PROFILE_SPECIFIER` are passed on the `xcodebuild` command line,
not written back into the project, so opening the project in Xcode still shows
automatic signing and nothing about the repository has to know about CI.
